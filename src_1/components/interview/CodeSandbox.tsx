...
export function CodeSandbox({ language, onChange, onOutputChange }: Props) {
  ...
  const handleRun = async () => {
    ...
    setOutput(result);
    setRunning(false);
    // Report output to parent so it can be included in evaluation
    if (onOutputChange) {
  ...
  return (
    <div className="space-y-3">
      ...
      >
        ...
        {running ? (loadingMsg || 'Running…') : 'Run Code'}
      </button>
      ...
      {output && (
        <div className="bg-[#0a0a0f] border border-[#2a2a38] rounded-xl p-4 space-y-2">
          ...
          {output.stdout && (
            <div>
              <div className="text-[10px] text-gray-400 mb-1 font-semibold">OUTPUT:</div>
              <pre className="text-xs text-green-300 font-mono whitespace-pre-wrap bg-black/30 p-2 rounded">{output.stdout}</pre>
            </div>
          )}
          {output.stderr && (
            <div>
              <div className="text-[10px] text-gray-400 mb-1 font-semibold">ERROR:</div>
              <pre className="text-xs text-red-400 font-mono whitespace-pre-wrap bg-black/30 p-2 rounded">{output.stderr}</pre>
            </div>
          )}
        </div>
      )}
