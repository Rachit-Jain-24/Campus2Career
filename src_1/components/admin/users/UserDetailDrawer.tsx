...
const statusColor = (s: string) => {
    switch (s) {
        ...
        case 'inactive': return 'text-muted-foreground bg-secondary/70 border-border';
            ...
            <div className="fixed top-0 right-0 h-full w-full max-w-lg bg-background border-l border-border z-50 overflow-y-auto animate-slide-in-right">
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500 to-fuchsia-500 flex items-center justify-center text-foreground text-xl font-bold flex-shrink-0">
                    ...
                    <div className="p-4 bg-card border border-border rounded-xl space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3 text-sm">
                            <div>
                                <div className="text-muted-foreground text-xs mb-0.5">User ID</div>
                                <div className="text-foreground font-mono text-xs">{user.id}</div>
                            </div>
                            {user.uid && (
                                <div>
                                    <div className="text-muted-foreground text-xs mb-0.5">Auth UID</div>
                                    <div className="text-foreground font-mono text-xs">{user.uid}</div>
                                </div>
                            )}
                            {user.phone && (
                                <div>
                    ...
                    <div className="p-4 bg-card border border-border rounded-xl space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3 text-sm">
                            <div>
                                ...
                                <div className="text-foreground font-medium">{user.department}</div>
                            </div>
                            <div>
                                <div className="text-muted-foreground text-xs mb-0.5">Role</div>
                                <div className="text-foreground font-medium capitalize">{user.role.replace(/_/g, ' ')}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-4 bg-card border border-border rounded-xl space-y-3">
                        ...
                        <div className="grid grid-cols-1 gap-2 text-sm">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-foreground text-xs font-medium">{formatDate(user.createdAt)}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <Clock className="w-3.5 h-3.5 text-slate-600" />
                                <span className="text-muted-foreground text-xs">Last Login:</span>
                                <span className="text-foreground text-xs font-medium">{formatDate(user.lastLogin)}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    {user.notes && (
                        <div className="p-4 bg-card border border-border rounded-xl space-y-2">
                            ...
                            <p className="text-sm text-foreground leading-relaxed">{user.notes}</p>
                        </div>
                    )}
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-foreground rounded-lg text-sm font-bold transition-colors shadow-lg shadow-indigo-500/20"
                    >
