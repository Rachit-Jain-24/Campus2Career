...
export async function seedBatchAccounts() {
    ...
    // Initialize a secondary app for student creation so it doesn't log out the current user
    ...
    for (let i = 0; i < batchStudentsData.length; i++) {
        ...
        try {
            ...
            const userProfile = {
                ...
                // Additional info
                bio: `Final year B.Tech CSDS student at NMIMS Hyderabad. Passionate about ${student.skills.machineLearning >= 8 ? 'Machine Learning' : student.skills.genAI >= 8 ? 'GenAI' : 'Data Science'} and building innovative solutions.`,
            ...
            // Save to Firestore with SAP ID as document ID in the correct 'students' collection
...
export async function deleteBatchAccounts() {
    ...
    for (const student of batchStudentsData) {
        try {
            ...
            // Delete Firestore document from 'students'
            await deleteDoc(doc(db, 'students', sapId));
