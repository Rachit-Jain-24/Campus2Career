...
import { X, Calendar, MapPin, Briefcase, FileText, Clock, Users, CheckCircle2, ExternalLink, Sparkles } from 'lucide-react';
import type { AdminDriveProfile } from '../../../types/driveAdmin';
import { applicationsDb } from '../../../services/db/database.service';
...
export const DriveDetailDrawer: React.FC<DriveDetailDrawerProps> = ({
    ...
    const [applicants, setApplicants] = useState<(DriveApplication & { student?: any })[]>([]);
    ...
    useEffect(() => {
        if (isOpen && drive && activeTab === 'applicants') {
            const loadApplicants = async () => {
                setIsLoadingApps(true);
                try {
                    const data = await applicationsDb.getDriveApplications(drive.id);
                    setApplicants(data);
                } catch (error) {
    ...
    const handleUpdateStatus = async (appId: string, status: string) => {
        try {
            await applicationsDb.updateApplicationStatus(appId, status);
        ...
        <>
            ...
            <div className="fixed inset-y-0 right-0 w-full md:w-[600px] bg-card border-l border-border/50 shadow-2xl z-50 overflow-y-auto animate-slide-in-right custom-scrollbar">
                ...
                <div className="sticky top-0 z-10 bg-card/95 backdrop-blur-xl border-b border-border">
                    ...
                    {/* Tab Navigation */}
                    <div className="flex px-6 border-t border-border/50">
                        <button 
                            onClick={() => setActiveTab('details')}
                            className={`px-4 py-3 text-xs font-bold uppercase tracking-widest transition-all border-b-2 ${activeTab === 'details' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
                        >
                            Drive Details
                        </button>
                        <button 
                            onClick={() => setActiveTab('applicants')}
                            className={`px-4 py-3 text-xs font-bold uppercase tracking-widest transition-all border-b-2 ${activeTab === 'applicants' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
                        >
                ...
                <div className="p-6 space-y-8 pb-12">
                    {activeTab === 'details' ? (
                        <>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center gap-3 text-sm text-foreground">
                                    ...
                                    <span>Registration: <span className="text-foreground font-medium">{(drive.registrationStart as Date)?.toLocaleDateString?.() || 'TBD'}</span> to <span className="text-foreground font-medium">{(drive.registrationEnd as Date)?.toLocaleDateString?.() || 'TBD'}</span></span>
                                </div>
                            </div>
                                                            ...
                                                            {stage.date && (
                                                                <div className="text-right flex-shrink-0">
                                                                    <span className="block text-xs font-medium text-muted-foreground">
                                                                        {stage.date instanceof Date ? stage.date.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : 'TBD'}
                                                                    </span>
                                                                </div>
                                                            )}
                                                        </div>
                        ...
                        <div className="space-y-4">
                            <h3 className="text-sm font-bold text-foreground flex items-center gap-2 border-b border-border pb-2">
                                <Users className="w-4 h-4 text-brand-500" /> Drive Applicants
                            </h3>

                            {isLoadingApps ? (
                                <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
                                    <div className="h-8 w-8 border-2 border-primary/20 border-t-primary rounded-full animate-spin mb-4" />
                                    <p className="text-sm font-medium">Fetching applicant data...</p>
                                </div>
                            ) : applicants.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-12 text-muted-foreground bg-secondary/20 rounded-2xl border border-border/50">
                                ...
                                <div className="space-y-3">
                                    {applicants.map((app) => (
                                        <div key={app.id} className="bg-secondary/30 border border-border/50 rounded-xl p-4 hover:bg-secondary/50 transition-all group">
                                            <div className="flex items-start justify-between gap-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <h4 className="text-sm font-black text-foreground">{app.student?.name || 'Unknown Student'}</h4>
                                                        <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">
                                                            {app.student?.sap_id} • {app.student?.branch}
                                                        </p>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="mt-4 flex items-center justify-between border-t border-border/30 pt-3">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <select 
                                                        ...
                                                        className="bg-card border border-border/50 rounded px-2 py-1 text-[10px] font-bold text-foreground focus:outline-none focus:ring-1 focus:ring-primary"
                                                    >
