...
export const CompanyTable: React.FC<CompanyTableProps> = ({
    ...
    const getStatusTheme = (status: string) => {
        switch (status) {
            ...
            case 'inactive':
            default: return { bg: 'bg-slate-500/10', text: 'text-muted-foreground', border: 'border-slate-500/20', Icon: AlertCircle };
        }
    };
    ...
    if (isLoading) {
        return (
            <div className="w-full bg-secondary/50 border border-border/50 rounded-xl p-8 flex flex-col items-center justify-center min-h-[400px]">
                ...
                <p className="mt-4 text-muted-foreground font-medium">Loading partner network...</p>
            </div>
        );
    }
                ...
                <table className="w-full text-left border-collapse min-w-[1000px]">
                    <thead>
                        <tr className="bg-card/50 border-b border-border/50 text-xs uppercase tracking-wider text-muted-foreground font-semibold">
                            <th
                                className="px-5 py-4 cursor-pointer group hover:bg-secondary/50 transition-colors w-1/4"
                                ...
                                >
                                    ...
                                    <td className="px-5 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-12 h-12 flex-shrink-0 bg-card border border-border rounded-xl flex items-center justify-center p-2 opacity-80 group-hover:opacity-100 transition-opacity">
                                                {company.logoUrl ? (
                                                    <img src={company.logoUrl} alt={company.companyName} className="max-w-full max-h-full object-contain" />
                                                ) : (
                                                    <span className="text-xl font-bold text-muted-foreground font-serif">
                                                        {company.companyName.charAt(0)}
                                                    </span>
                                                )}
                                            </div>
                                            <div className="min-w-0">
                                                <div className="font-bold text-foreground group-hover:text-brand-300 transition-colors truncate">
                                                    {company.companyName}
                                                </div>
