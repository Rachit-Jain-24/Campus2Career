import React, { useEffect } from 'react';
import { X, Mail, Phone, BookOpen, Briefcase, Award, TrendingUp, Download, Building2 } from 'lucide-react';
import { SWOCAnalysis } from '../../ui/SWOCAnalysis';
import type { AdminStudentProfile } from '../../../types/studentAdmin';

interface StudentDetailDrawerProps {
    student: AdminStudentProfile | null;
    isOpen: boolean;
    onClose: () => void;
}

export const StudentDetailDrawer: React.FC<StudentDetailDrawerProps> = ({
    student,
    isOpen,
    onClose
}) => {

    // Simple mapper to make AdminStudentProfile compatible with SWOCAnalysis
    const mappedStudentForSWOC = student ? {
        name: student.fullName,
        branch: student.department,
        currentYear: student.currentYear,
        cgpa: student.cgpa,
        techSkills: student.skills,
        projects: student.projectsCount ? new Array(student.projectsCount) : [],
        internships: student.internshipCompleted ? [1] : [],
        careerTrack: student.careerGoal,
        assessmentResults: (student as any).assessmentResults || {}
    } : null;

    // Handle Escape key to close
    useEffect(() => {
        const handleEsc = (e: KeyboardEvent) => {
            if (e.key === 'Escape') onClose();
        };
        if (isOpen) {
            window.addEventListener('keydown', handleEsc);
        }
        return () => window.removeEventListener('keydown', handleEsc);
    }, [isOpen, onClose]);

    if (!isOpen || !student) return null;

    return (
        <>
            {/* Backdrop */}
            <div
                className="fixed inset-0 bg-background/60 backdrop-blur-sm z-40 transition-opacity animate-fade-in"
                onClick={onClose}
            />

            {/* Drawer */}
            <div className="fixed inset-y-0 right-0 w-full md:w-[600px] bg-card border-l border-border/50 shadow-2xl z-50 overflow-y-auto animate-slide-in-right text-slate-900 leading-normal">

                {/* Header Section */}
                <div className="sticky top-0 z-10 bg-card/90 backdrop-blur-md border-b border-border p-6 flex items-start justify-between">
                    <div className="flex items-center gap-4">
                        <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-primary/20 to-purple-500/20 border border-primary/20 flex items-center justify-center shadow-inner">
                            <span className="text-2xl font-bold text-primary">
                                {student.fullName.charAt(0)}{student.fullName.split(' ')[1]?.[0] || ''}
                            </span>
                        </div>
                        <div>
                            <h2 className="text-2xl font-bold text-foreground">{student.fullName}</h2>
                            <p className="text-muted-foreground font-mono mt-1">{student.sapId}</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                        className="p-2 text-muted-foreground hover:text-foreground hover:bg-secondary rounded-lg transition-colors"
                    >
                        <X className="w-5 h-5" />
                    </button>
                </div>

                {/* Content Body */}
                <div className="p-6 space-y-8 pb-32">

                    {/* Quick Stats Grid */}
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <div className="bg-secondary/50 border border-border/50 rounded-xl p-4 text-center">
                            <div className="text-sm text-muted-foreground mb-1">CGPA</div>
                            <div className="text-xl font-bold text-primary">{typeof student.cgpa === 'number' ? student.cgpa.toFixed(2) : (Number(student.cgpa) || 0).toFixed(2)}</div>
                        </div>
                        <div className="bg-secondary/50 border border-border/50 rounded-xl p-4 text-center">
                            <div className="text-sm text-muted-foreground mb-1">Readiness</div>
                            <div className="text-xl font-bold text-foreground">{student.readinessScore}%</div>
                        </div>
                        <div className="bg-secondary/50 border border-border/50 rounded-xl p-4 text-center">
                            <div className="text-sm text-muted-foreground mb-1">Projects</div>
                            <div className="text-xl font-bold text-foreground">{student.projectsCount || 0}</div>
                        </div>
                        <div className="bg-secondary/50 border border-border/50 rounded-xl p-4 text-center">
                            <div className="text-sm text-muted-foreground mb-1">Offers</div>
                            <div className="text-xl font-bold text-emerald-400">{student.offersCount || 0}</div>
                        </div>
                    </div>

                    {/* AI SWOC Analysis for Admins */}
                    <div className="bg-secondary/10 border border-border/20 rounded-[2.5rem] p-6 shadow-sm">
                        <SWOCAnalysis 
                            studentData={mappedStudentForSWOC} 
                            editable={true} 
                        />
                    </div>

                    {/* Contact Info */}
                    <div className="bg-secondary/30 border border-border/30 rounded-xl p-5">
                        <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider mb-4">Contact & Basics</h3>
                        <div className="space-y-3">
                            <div className="flex items-center gap-3 text-muted-foreground">
                                <Mail className="w-4 h-4 text-muted-foreground" />
                                <a href={`mailto:${student.email}`} className="hover:text-primary transition-colors">{student.email}</a>
                            </div>
                            {student.contact && (
                                <div className="flex items-center gap-3 text-muted-foreground">
                                    <Phone className="w-4 h-4 text-muted-foreground" />
                                    <span>{student.contact}</span>
                                </div>
                            )}
                            <div className="flex items-center gap-3 text-muted-foreground">
                                <Building2 className="w-4 h-4 text-muted-foreground" />
                                <span>{student.department} • {student.currentYear}</span>
                            </div>
                        </div>
                    </div>

                    {/* Readiness Gauge & Goal */}
                    <div className="bg-secondary/30 border border-border/30 rounded-xl p-5">
                        <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider mb-4 flex items-center gap-2">
                            <TrendingUp className="w-4 h-4 text-primary" />
                            Placement Readiness
                        </h3>

                        <div className="mb-6">
                            <div className="flex justify-between items-end mb-2">
                                <span className="text-muted-foreground text-sm">Overall Score</span>
                                <span className="text-2xl font-bold text-foreground">{student.readinessScore}%</span>
                            </div>
                            <div className="w-full h-3 bg-card rounded-full overflow-hidden border border-border/50">
                                <div
                                    className={`h-full rounded-full transition-all duration-1000 ${student.readinessScore >= 75 ? 'bg-emerald-500' :
                                            student.readinessScore >= 50 ? 'bg-amber-500' : 'bg-rose-500'
                                        }`}
                                    style={{ width: `${student.readinessScore}%` }}
                                />
                            </div>
                        </div>

                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <span className="block text-xs text-muted-foreground mb-1">Career Goal</span>
                                <span className="inline-block px-3 py-1 bg-card border border-border rounded-lg text-sm text-foreground">
                                    {student.careerGoal}
                                </span>
                            </div>
                            <div>
                                <span className="block text-xs text-muted-foreground mb-1">Mandatory Internship</span>
                                <span className={`inline-block px-3 py-1 rounded-lg text-sm border ${student.internshipCompleted
                                        ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400'
                                        : 'bg-rose-500/10 border-rose-500/20 text-rose-400'
                                    }`}>
                                    {student.internshipCompleted ? 'Completed' : 'Pending'}
                                </span>
                            </div>
                        </div>
                    </div>

                    {/* Resume & Documents */}
                    <div className="bg-secondary/30 border border-border/30 rounded-xl p-5 flex items-center justify-between text-slate-900">
                        <div>
                            <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider mb-1 flex items-center gap-2">
                                <BookOpen className="w-4 h-4 text-muted-foreground" />
                                Master Resume
                            </h3>
                            <p className="text-sm text-muted-foreground">Status: <span className="capitalize">{student.resumeStatus}</span></p>
                        </div>
                        <button
                            disabled={student.resumeStatus === 'not_uploaded'}
                            className="flex items-center gap-2 px-4 py-2 bg-secondary hover:bg-secondary disabled:opacity-50 disabled:cursor-not-allowed border border-slate-600 rounded-lg text-sm font-medium text-foreground transition-colors"
                        >
                            <Download className="w-4 h-4" />
                            Download
                        </button>
                    </div>

                    {/* Skills & Certifications */}
                    <div className="space-y-6">
                        {student.skills && student.skills.length > 0 && (
                            <div>
                                <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider mb-3 flex items-center gap-2">
                                    <Briefcase className="w-4 h-4 text-muted-foreground" />
                                    Technical Skills
                                </h3>
                                <div className="flex flex-wrap gap-2">
                                    {student.skills.map((skill, idx) => (
                                        <span key={idx} className="px-3 py-1 bg-secondary/80 border border-border rounded-full text-sm text-foreground">
                                            {typeof skill === 'string' ? skill : (skill as any).name || 'Unknown Skill'}
                                        </span>
                                    ))}
                                </div>
                            </div>
                        )}

                        {student.certifications && student.certifications.length > 0 && (
                            <div>
                                <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider mb-3 flex items-center gap-2">
                                    <Award className="w-4 h-4 text-muted-foreground" />
                                    Certifications
                                </h3>
                                <ul className="space-y-2">
                                    {student.certifications.map((cert, idx) => {
                                        const isObject = typeof cert !== 'string';
                                        return (
                                            <li key={idx} className="flex items-start gap-3 p-3 bg-secondary/30 border border-border/50 rounded-lg">
                                                <Award className="w-5 h-5 text-primary shrink-0 mt-0.5" />
                                                <div className="flex flex-col">
                                                    <span className="text-sm font-bold text-foreground">
                                                        {isObject ? cert.name : cert}
                                                    </span>
                                                    {isObject && (
                                                        <span className="text-xs text-muted-foreground mt-0.5">
                                                            {cert.issuer} {cert.year ? `• ${cert.year}` : ''}
                                                        </span>
                                                    )}
                                                </div>
                                            </li>
                                        );
                                    })}
                                </ul>
                            </div>
                        )}
                        {student.achievements && student.achievements.length > 0 && (
                            <div>
                                <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider mb-3 flex items-center gap-2">
                                    <Award className="w-4 h-4 text-amber-500" />
                                    Achievements & Honors
                                </h3>
                                <div className="space-y-2">
                                    {student.achievements.map((ach, idx) => (
                                        <div key={idx} className="p-3 bg-amber-500/5 border border-amber-500/10 rounded-lg">
                                            <p className="text-sm font-bold text-foreground">{ach.title}</p>
                                            <p className="text-xs text-muted-foreground mt-1">{ach.description}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        )}

                        {/* Projects Section */}
                        {(student as any).projects && (student as any).projects.length > 0 && (
                            <div>
                                <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider mb-3 flex items-center gap-2">
                                    <Briefcase className="w-4 h-4 text-blue-500" />
                                    Academic & Personal Projects
                                </h3>
                                <div className="space-y-3">
                                    {(student as any).projects.map((proj: any, idx: number) => (
                                        <div key={idx} className="p-4 bg-blue-500/5 border border-blue-500/10 rounded-xl">
                                            <p className="text-sm font-bold text-foreground">{proj.title}</p>
                                            <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{proj.description}</p>
                                            <div className="flex flex-wrap gap-1.5 mt-2">
                                                {(proj.tech || proj.tech_stack || '').split(',').map((t: string) => t.trim()).filter(Boolean).map((t: string) => (
                                                    <span key={t} className="px-1.5 py-0.5 bg-blue-500/10 text-blue-400 text-[10px] font-bold rounded uppercase">
                                                        {t}
                                                    </span>
                                                ))}
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        )}

                        {/* Internships Section */}
                        {(student as any).internships && (student as any).internships.length > 0 && (
                            <div>
                                <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider mb-3 flex items-center gap-2">
                                    <Building2 className="w-4 h-4 text-emerald-500" />
                                    Work Experience & Internships
                                </h3>
                                <div className="space-y-3">
                                    {(student as any).internships.map((intern: any, idx: number) => (
                                        <div key={idx} className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-xl">
                                            <div className="flex justify-between items-start">
                                                <p className="text-sm font-bold text-foreground">{intern.company || intern.companyName}</p>
                                                <span className="text-[10px] font-bold text-muted-foreground uppercase">{intern.period || intern.duration}</span>
                                            </div>
                                            <p className="text-xs text-emerald-400 font-bold mt-0.5 uppercase tracking-widest">{intern.role}</p>
                                            <p className="text-xs text-muted-foreground mt-2 line-clamp-2">{intern.description}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        )}
                    </div>
                </div>
            </div>
        </>
    );
};
