  ...
  const handleResume = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!text?.trim() || text.trim().length < 50) {
        showToast('Could not extract text from this PDF. Ensure it is a text-based (not scanned) PDF.', 'error');
        return;
      }
      setResumeName(file.name);
      setResumeText(text);
    } catch (err) {
      showToast('Failed to parse PDF. Ensure it is a valid, text-based PDF file.', 'error');
    } finally {
      setIsParsing(false);
      e.target.value = '';
    }
