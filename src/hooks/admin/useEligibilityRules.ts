...
export const useEligibilityRules = () => {
    ...
    const fetchRules = useCallback(async () => {
        ...
        try {
            ...
            setRules(data);
            if (data.length === 0) {
                logger.info('Firestore eligibility_rules collection is empty.');
            }
        } catch (err: any) {
            logger.error('Error fetching eligibility rules', { error: err });
    ...
    const fetchAttachableDrives = useCallback(async () => {
        ...
        try {
            ...
            logger.error('Error fetching drives for eligibility attachment', { error: err });
    ...
    const attachRuleToDrive = useCallback(async (rule: AdminEligibilityRule, driveId: string) => {
        ...
        try {
            ...
            logger.error('Attach to drive failed', { error: err });
