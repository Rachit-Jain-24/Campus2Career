...
export const useDrives = () => {
    ...
    const fetchDrives = useCallback(async () => {
        try {
            ...
            setDrives(data);
            if (data.length === 0) {
                logger.info('Firestore drives collection is empty.');
            }
        } catch (err: any) {
            logger.error('Failed to fetch drives from Firestore', { error: err?.message });
    ...
    useEffect(() => {
        fetchDrives();
        const unsubscribe = drivesService.onDrivesChange(setDrives);
        return () => unsubscribe();
    }, [fetchDrives]);
    ...
    const handleSaveDrive = async (formData: DriveFormData) => {
        ...
        try {
            ...
            logger.error('Failed to save drive', { error: err });
