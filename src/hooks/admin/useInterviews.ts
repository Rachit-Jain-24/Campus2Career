...
export const useInterviews = () => {
    ...
    const fetchInterviews = useCallback(async () => {
        ...
        try {
            ...
            setInterviews(data);
            if (data.length === 0) {
                logger.info('Firestore interviews collection is empty.');
            }
        } catch (err: any) {
            logger.error('Error fetching interview schedules', { error: err });
    ...
    useEffect(() => {
        fetchInterviews();
        const unsubscribe = interviewsService.onInterviewsChange(setInterviews);
        return () => unsubscribe();
    }, [fetchInterviews]);
