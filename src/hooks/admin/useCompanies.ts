...
import { fetchAllCompanies, createCompany, updateCompany, onCompaniesChange } from '../../services/admin/companies.service';
...
export const useCompanies = () => {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            setCompanies(data);
            if (data.length === 0) {
                logger.info('Firestore companies collection is empty.');
            }
        } catch (err: unknown) {
            logger.error('Failed fetching companies', { error: err });
    ...
    useEffect(() => {
        loadData();
        const unsubscribe = onCompaniesChange(setCompanies);
        return () => unsubscribe();
    }, [loadData]);
