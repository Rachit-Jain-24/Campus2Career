...
import { 
    studentsDb, companiesDb, drivesDb, interviewsDb, 
    offersDb, auditDb 
...
export const useDashboardData = (): DashboardData => {
    ...
    const fetchAll = useCallback(async () => {
        ...
        try {
            ...
            if (failures.length > 0) {
                logger.warn(`${failures.length} of ${results.length} dashboard data sources failed to load.`);
            }
        } catch (err: any) {
    ...
    useEffect(() => {
        const unsubStudents = studentsDb.onStudentsChange(setStudents);
        const unsubCompanies = companiesDb.onCompaniesChange(setCompanies);
        const unsubDrives = drivesDb.onDrivesChange(setDrives);
        const unsubInterviews = interviewsDb.onInterviewsChange(setInterviews);
        const unsubOffers = offersDb.onOffersChange(setOffers);
        const unsubAudit = auditDb.onAuditLogsChange(setAuditLogs);
