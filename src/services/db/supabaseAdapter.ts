...
import type { 
  ...
  NotificationAdapter, ApplicationAdapter, DriveApplication, SystemNotification as Notification, Unsubscribe,
...
const mapStudentData = (s: any): AdminStudentProfile => ({
  ...
  internshipCompleted: (s.internships?.length > 0) || (s.internship_list?.length > 0) || false,
  contact: s.phone || '',
  skills: s.skills || [],
  certifications: s.certification_list || s.certifications || [],
  projectsCount: s.projects_count || s.projects?.length || 0,
  offersCount: 0,
  achievements: s.achievements || [],
  bio: s.bio || '',
  projects: s.project_list || s.projects || [],
  internships: s.internship_list || s.internships || [],
  leetcodeStats: {
...
export const supabaseStudents: StudentAdapter = {
  ...
  onStudentsChange(callback: (students: AdminStudentProfile[]) => void): () => void {
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'student_internships' }, async () => {
        const students = await this.fetchAllStudents();
        callback(students);
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'student_certifications' }, async () => {
        const students = await this.fetchAllStudents();
        callback(students);
      })
      .subscribe();
...
export const supabaseCompanies: CompanyAdapter = {
  ...
  onCompaniesChange(callback: (companies: AdminCompanyProfile[]) => void): () => void {
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'company_departments' }, async () => {
        const companies = await this.fetchAllCompanies();
        callback(companies);
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'company_job_roles' }, async () => {
        const companies = await this.fetchAllCompanies();
        callback(companies);
      })
      .subscribe();
...
export const supabaseDrives: DriveAdapter = {
  async fetchAllDrives(): Promise<AdminDriveProfile[]> {
    ...
    return data.map(d => {
      const eligibility = d.eligibility_rules || {};
      return {
        id: d.id,
        ...
        eligibilityRules: {
          ...
          allowedYears: eligibility.allowedYears || [],
          requiresResumeApproval: eligibility.requiresResumeApproval || false,
          mandatoryInternship: eligibility.mandatoryInternship || false,
          requiredSkills: eligibility.requiredSkills || [],
          minCGPA: eligibility.minCGPA || 0,
          maxActiveBacklogs: eligibility.maxActiveBacklogs || 0,
          maxHistoryBacklogs: eligibility.maxHistoryBacklogs || 0,
        },
  ...
  async createDrive(formData: DriveFormData): Promise<string> {
    ...
    try {
      await supabaseNotifications.sendNotification({
        role: 'student',
        title: 'New Drive Announced! 🚀',
        message: `${formData.companyName} is hiring for ${formData.jobRole || formData.title}. Register now!`,
        type: 'new_drive',
        link: '/student/drives'
      });
    } catch (err) {
      console.warn('[supabaseAdapter] Failed to send drive notification:', err);
    }
...
export const supabaseInterviews: InterviewAdapter = {
  ...
  async getStudentInterviews(studentId: string, sapId?: string): Promise<AdminInterview[]> {
    let query = supabase
      .from('interviews')
      .select('*');
    
    // student_id is a UUID column in the interviews table. 
    // We cannot query it with a SAP ID string or it will throw a 'invalid input syntax for type uuid' error.
  ...
  onInterviewsChange(callback: (interviews: AdminInterview[]) => void): () => void {
    const channel = supabase
      .channel('public:interviews')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'interviews' }, async () => {
...
export const supabaseAudit: AuditAdapter = {
  ...
  onAuditLogsChange(callback: (logs: AuditLogEntry[]) => void): () => void {
    const channel = supabase
...
export const supabaseUser: UserAdapter = {
  async lookupUserProfileByEmail(email: string): Promise<any | null> {
    try {
      const isStaffDomain = email.endsWith('@nmims.edu');
      const isStudentDomain = email.endsWith('@nmims.edu.in');

      // Create lookup promises (Using ilike for case-insensitive matching)
      const lookupAdminPromise = withTimeout(supabase.from('admins').select('*').ilike('email', email).maybeSingle() as any, 6000);
      const lookupStudentRawPromise = withTimeout(supabase.from('students').select('*').ilike('email', email).maybeSingle() as any, 6000);
      const lookupStudentViewPromise = withTimeout(supabase.from('student_readiness_v').select('*').ilike('email', email).maybeSingle() as any, 8000);

      // Step 1: Check primary target based on domain (Fast path)
      try {
        if (isStaffDomain) {
          const { data: admin } = await lookupAdminPromise;
          if (admin) {
            console.log(`[SupabaseAdapter] Admin found via fast-path for ${email}`);
            return {
              ...admin,
          ...
          const { data: rawStudent } = await lookupStudentRawPromise;
          if (rawStudent) {
            console.log(`[SupabaseAdapter] Student found via fast-path for ${email}`);
            return mapStudentRow(rawStudent);
          }
        }
      } catch (e: any) {
        if (e.message !== 'QUERY_TIMEOUT') throw e;
        console.warn(`[SupabaseAdapter] Fast-path timed out for ${email}`);
      }

      // Step 2: Fallback to parallel check
      console.log(`[SupabaseAdapter] Running full source scan for ${email}...`);
      const results = await Promise.allSettled([lookupStudentRawPromise, lookupStudentViewPromise, lookupAdminPromise]);

      const rawStudentRes = results[0].status === 'fulfilled' ? (results[0].value as any).data : null;
      const studentViewRes = results[1].status === 'fulfilled' ? (results[1].value as any).data : null;
      const adminRes = results[2].status === 'fulfilled' ? (results[2].value as any).data : null;

      if (adminRes) {
        console.log(`[SupabaseAdapter] Profile found in admins table for ${email}`);
        return {
          ...adminRes,
          uid: adminRes.uid || adminRes.id,
          name: adminRes.full_name || adminRes.name,
          role: adminRes.role || 'system_admin',
        };
      }

      if (studentViewRes) {
        console.log(`[SupabaseAdapter] Profile found in student_readiness_v for ${email}`);
        return mapStudentRow(studentViewRes);
      }

      if (rawStudentRes) {
        console.log(`[SupabaseAdapter] Profile found in students table for ${email}`);
        return mapStudentRow(rawStudentRes);
      }

      // If all checks finished but no data found
      const errors = results.filter(r => r.status === 'rejected');
      if (errors.length > 0) {
        const hasTimeout = errors.some(e => (e as any).reason?.message === 'QUERY_TIMEOUT');
        if (hasTimeout) throw new Error('QUERY_TIMEOUT');
      }

      console.warn(`[SupabaseAdapter] No profile found in ANY table for ${email}`);
      return null;
    } catch (err: any) {
      if (err.message === 'QUERY_TIMEOUT') throw err;
      console.warn('[SupabaseAdapter] Profile lookup error:', err.message);
      return null;
    }
  },
  ...
  async updateUser(col: string, id: string, data: any): Promise<void> {
    ...
    if (table === 'students') {
      ...
      if (data.uid !== undefined) {
        // mapped.uid = data.uid; // REMOVED
        // mapped.id = data.uid; // REMOVED
      }
...
// --- Application Adapter ---
export const supabaseApplications: ApplicationAdapter = {
  async applyForDrive(data: Partial<DriveApplication>): Promise<void> {
    try {
      const { error } = await supabase
      ...
      // Manually increment applicant_count on drives table
      if (data.driveId) {
          ...
          if (driveData) {
              await supabase
                  .from('drives')
                  .update({ applicant_count: (driveData.applicant_count || 0) + 1 })
                  .eq('id', data.driveId);
          }
      }

    } catch (err) {
      console.warn('[supabaseAdapter] Failed to apply for drive:', err);
    }
    ...
    try {
      await supabaseNotifications.sendNotification({
        ...
        message: `A student has applied for a drive. Check the dashboard.`,
        type: 'application_received',
        link: '/admin/drives'
      });
    } catch (err) {
      console.warn('[supabaseAdapter] Failed to notify admin:', err);
    }
  },

  async getStudentApplications(studentId: string): Promise<DriveApplication[]> {
    try {
      const { data, error } = await supabase
        .from('drive_applications')
        .select('*')
      ...
      console.warn('[supabaseAdapter] getStudentApplications failed:', err);
      return [];
    }
  },

  async getDriveApplications(driveId: string): Promise<DriveApplication[]> {
    try {
      const { data, error } = await supabase
        .from('drive_applications')
        .select('*, students(name, sap_id, branch, current_year)')
        .eq('drive_id', driveId)
      ...
      return (data || []).map(a => ({
        ...
        matchScore: a.match_score,
        feedback: a.feedback,
        student: Array.isArray(a.students) ? a.students[0] : (a.students || a.student || null)
      }));
    } catch (err) {
      console.warn('[supabaseAdapter] getDriveApplications failed:', err);
      return [];
    }
  },
  ...
  async updateApplicationStatus(id: string, status: string, feedback?: string): Promise<void> {
    try {
      const { data: app, error: fetchError } = await supabase
        .from('drive_applications')
        .select('student_id, drive_id, drives(company_name)')
        .eq('id', id)
        .single();
      ...
      // Manually increment shortlisted_count on drives table
      if (status === 'shortlisted' && app && app.drive_id) {
          const { data: driveData } = await supabase
              .from('drives')
              .select('shortlisted_count')
              .eq('id', app.drive_id)
              .single();
          ...
          if (driveData) {
              await supabase
                  .from('drives')
                  .update({ shortlisted_count: (driveData.shortlisted_count || 0) + 1 })
                  .eq('id', app.drive_id);
          }
      }
      ...
      if (app) {
        try {
          await supabaseNotifications.sendNotification({
            ...
            message: `Your application for ${(app as any).drives?.company_name} has been marked as ${status}.`,
            type: 'application_status',
            link: '/student/drives'
          });
        } catch (err) {
          console.warn('[supabaseAdapter] Failed to notify student:', err);
        }
      }
    } catch (err) {
      console.warn('[supabaseAdapter] updateApplicationStatus failed:', err);
    }
  },

  onApplicationsChange(callback: (apps: DriveApplication[]) => void): Unsubscribe {
    const channel = supabase
      .channel('public:drive_applications')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'drive_applications' }, async () => {
        // This is a global listener, might need filtering in the hook
        callback([]); // Trigger re-fetch
      })
      .subscribe();
...
export const supabaseNotifications: NotificationAdapter = {
  async getNotifications(userId: string, role: string): Promise<Notification[]> {
    try {
      let query = supabase.from('notifications').select('*').order('created_at', { ascending: false });
      
      // If admin, get their specific notifications AND broadcast notifications (user_id is null, role is admin)
      if (['admin', 'placement_officer', 'dean', 'faculty', 'system_admin', 'director'].includes(role)) {
          query = query.or(`user_id.eq.${userId},and(user_id.is.null,role.eq.admin)`);
      } else {
      ...
      console.warn('[supabaseAdapter] getNotifications failed:', err);
      return [];
    }
  },
  ...
  async markAsRead(id: string): Promise<void> {
    try {
      ...
      console.warn('[supabaseAdapter] markAsRead failed:', err);
    }
  },
  ...
  async markAllAsRead(userId: string, role: string): Promise<void> {
    try {
      ...
      console.warn('[supabaseAdapter] markAllAsRead failed:', err);
    }
  },
  ...
  async sendNotification(data: Partial<Notification>): Promise<void> {
    try {
      ...
      console.warn('[supabaseAdapter] sendNotification failed:', err);
    }
  },

  onNotificationsChange(userId: string, role: string, callback: (notifications: Notification[]) => void): Unsubscribe {
    // Determine the filter string based on role
    const isAdmin = ['admin', 'placement_officer', 'dean', 'faculty', 'system_admin', 'director'].includes(role);
    
    // Note: Postgres changes filter only supports simple column=value filters. 
    // We can't do OR filters in the Realtime channel subscription directly easily.
    // As a workaround, we will just listen to ALL notifications if admin, or specific if student, 
    // and then let getNotifications do the exact filtering when fetching.
    const filter = isAdmin ? undefined : `user_id=eq.${userId}`;

    const channel = supabase
      .channel(`public:notifications:${userId}`)
      .on('postgres_changes', 
        { event: '*', schema: 'public', table: 'notifications', ...(filter ? { filter } : {}) }, 
        async () => {
          const notes = await this.getNotifications(userId, role);
          callback(notes);
        }
      )
      .subscribe();
