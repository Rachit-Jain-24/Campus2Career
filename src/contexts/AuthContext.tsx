...
/** Shape of the student registration form data */
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const resetTimer = () => {
            ...
            idleTimer = setTimeout(async () => {
                logger.debug('[AuthContext] Idle timeout — signing out');
    ...
    const lookupUserProfileByEmail = async (email: string): Promise<AppUser | null> => {
        try {
            ...
            logger.error('Error looking up user profile', { error });
            return null;
        }
    };
    ...
    useEffect(() => {
        ...
        authTimeout = setTimeout(() => {
            if (isInitializing) {
                logger.warn('[AuthContext] Auth initialization timed out. Forcing load.');
                setIsInitializing(false);
            }
        }, 15000);

        const handleUserHydration = async (email: string) => {
            logger.debug(`[AuthContext] Hydrating profile for: ${email}`);
            try {
                ...
                if (cachedUser) {
                    ...
                    logger.warn('[AuthContext] Failed to parse cached user from localStorage');
                }                }
                ...
                try {
                    ...
                    if (freshProfile) {
                        logger.debug(`[AuthContext] Profile fetched successfully for ${email}. Role: ${freshProfile.role}`);
                        setUser(freshProfile);
                        localStorage.setItem('c2c_user', JSON.stringify(freshProfile));
                    } else {
                        // Truly not found in DB -> Logout
                        logger.warn(`[AuthContext] Genuinely no DB profile found for ${email}. Signing out.`);
                        await supabase.auth.signOut();
                        setUser(null);
                        localStorage.removeItem('c2c_user');
                    }
                    ...
                    if (err.message === 'QUERY_TIMEOUT') {
                        logger.warn(`[AuthContext] Profile hydration timed out for ${email}. Staying with cached session.`);
                        // Cached user was already set above — don't sign out
                    } else {
                        logger.error('[AuthContext] Hydration error', { error: err });
                    }
                }
            } catch (error) {
                logger.error('[AuthContext] Critical hydration error', { error });
            } finally {
        ...
        const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
            logger.debug(`[Supabase Auth] Event: ${event} | Session: ${session ? 'Active' : 'None'}`);
            ...
            if (isPasswordRecovery) {
                logger.debug('[AuthContext] Password recovery flow detected - skipping profile hydration');
                clearTimeout(authTimeout);
                setIsInitializing(false);
                return;
            }
        ...
        try {
            ...
            try {
                await studentsDb.createStudent({
                    ...
                    rollNo: data.rollNo || '',
                    currentYear: data.currentYear ? parseInt(String(data.currentYear)) || 1 : 1,
                    batch: '',
                    cgpa: '0',
    ...
    const updateUser = async (newUser: AppUser) => {
        try {
            ...
            if (!docId) {
                logger.error('[updateUser] No docId found — uid is missing from user object');
                throw new Error('Cannot save: user ID is missing.');
            }

            logger.debug(`[updateUser] Saving to ${collectionName}/${docId}`, {
                internships: (newUser as StudentUser).internships?.length,
                projects: (newUser as StudentUser).projects?.length,
                techSkills: (newUser as StudentUser).techSkills?.length,
            });
            ...
            logger.debug('[updateUser] Save successful');
        } catch (error: any) {
