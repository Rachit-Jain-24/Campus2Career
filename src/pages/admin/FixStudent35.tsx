...
export default function FixStudent35() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      // 1. Delete previous student document if exists
      try {
        await deleteDoc(doc(db, 'students', sapId));
        message += "Deleted old Firestore doc. ";
      } catch (e) {
        message += "No old doc to delete. ";
      }
      ...
      // 3. Create fresh Firestore document from Excel data
      const studentData = {
        achievements: [
      ...
      await setDoc(doc(db, 'students', sapId), studentData);
