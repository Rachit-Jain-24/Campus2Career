...
import { drivesDb, interviewsDb } from '../../services/db/database.service';
...
export default function StudentDashboard() {
    ...
    const [upcomingInterviews, setUpcomingInterviews] = useState<AdminInterview[]>([]);
    ...
    useEffect(() => {
        ...
        const loadInterviews = async () => {
            try {
                const data = await interviewsDb.getStudentInterviews(user.id, user.sapId);
                const upcoming = data.filter(i =>
                    i.status === 'scheduled' || i.status === 'rescheduled' || i.status === 'completed'
                );
        ...
        const unsubscribe = interviewsDb.onInterviewsChange(() => {
            loadInterviews();
        });
    ...
    const getQuickStats = () => {
        ...
        stats.push({
            label: "Interviews",
            value: isInterviewsLoading ? "..." : upcomingInterviews.length.toString(),
            icon: Calendar,
            color: "text-blue-600",
            bg: "bg-blue-50",
            trend: upcomingInterviews.length > 0 ? "Check schedule" : "No active sessions",
            onClick: () => {
    ...
    const handleReSyncAI = async () => {
        ...
        try {
            const analysis = await analyzeProfileWithAI(user);
            const updatedUser = { 
                ...user, 
                resumeAnalysis: analysis,
                readinessScore: analysis.careerStrength 
            };
            // Use the global updateUser from useAuth so UI refreshes
            ...
            <div className="space-y-6">
                ...
                <div className="rounded-3xl bg-gradient-nmims p-8 text-white overflow-hidden relative shadow-2xl border border-white/10 group">
                    ...
                    <div className="relative z-10">
                        ...
                        <h1 className="text-4xl md:text-5xl font-black mb-2 text-white">
                            {getGreeting()}, <span className="text-white/90">{user?.name?.split(' ')[0]}!</span>
                        </h1>
                        <p className="text-white/80 text-lg font-medium max-w-xl">
                            {modeLabel} — Ready to take the next step in your career?
                        </p>
                    </div>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
                    {quickStats.map((stat, i) => (
                        <div 
                            key={i} 
                            onClick={() => stat.onClick ? stat.onClick() : stat.link ? navigate(stat.link) : null}
                            className={`group relative rounded-2xl border border-slate-100 bg-white p-4 flex items-start justify-between shadow-sm hover:shadow-xl transition-all ${ (stat.onClick || stat.link) ? 'cursor-pointer hover:border-primary/30' : ''}`}
                        >
                            <div className="relative z-10">
                                ...
                                <p className="text-[9px] font-bold text-slate-400 mt-1">{stat.trend}</p>
                            </div>
                ...
                {!(user as StudentUser)?.careerTrack ? (
                    <div className="relative rounded-3xl bg-white border-2 border-slate-100 p-6 overflow-hidden">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        {user?.resumeAnalysis ? (
                            <div className="relative rounded-2xl bg-slate-900 border border-slate-800 p-4 overflow-hidden group">
                                <div className="absolute top-0 right-0 p-2 opacity-10"><Sparkles className="h-10 w-10 text-white" /></div>
                                <div className="relative flex items-center justify-between gap-4">
                                    <div className="flex items-center gap-3">
                                        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/20 text-primary font-black text-sm border border-primary/30">
                                            {user.resumeAnalysis.careerStrength}
                                        </div>
                                        <div>
                                            <p className="font-bold text-sm text-white flex items-center gap-2">
                                                🤖 AI Insights: {user.resumeAnalysis.overallStatus}
                                                <Shield className="h-3 w-3 text-emerald-400" />
                                            </p>
                                            <p className="text-[10px] text-slate-400 truncate max-w-[200px]">Market Readiness: {user.resumeAnalysis.marketReadiness}</p>
                                        </div>
                                    </div>
                                    <div className="flex gap-2">
                                        <button 
                                            ...
                                            className="px-3 py-1.5 bg-white/10 hover:bg-white/20 text-white text-[10px] font-bold rounded-lg border border-white/10 transition-all flex items-center gap-1"
                                        >
                                            {isRefreshing ? <RefreshCw className="h-3 w-3 animate-spin" /> : <RefreshCw className="h-3 w-3" />}
                                            Update
                                        </button>
                                        <button onClick={() => navigate('/student/profile')} className="px-3 py-1.5 bg-primary text-white text-[10px] font-bold rounded-lg hover:bg-primary-dark transition-all">
                                            View Full Analysis
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div className="relative rounded-2xl bg-gradient-to-r from-primary/20 via-primary/10 to-transparent border border-primary/30 p-4 overflow-hidden">
                                <div className="relative flex items-center justify-between gap-4">
                                    <div className="flex items-start gap-3">
                                        <div className="rounded-xl bg-primary/20 p-2 shrink-0"><Sparkles className="h-5 w-5 text-primary" /></div>
                                        <div>
                                            <p className="font-bold text-sm text-slate-800">🤖 AI Profile Analysis</p>
                                            <p className="text-[11px] text-slate-500 mt-0.5">Upload your resume to get deep career insights.</p>
                                        </div>
                                    </div>
                                    <button onClick={() => navigate('/student/profile')} className="bg-primary/10 text-primary hover:bg-primary/20 transition-colors px-3 py-1.5 text-[10px] font-bold rounded-lg border border-primary/20">
                                        Analyze Resume →
                                    </button>
                                </div>
                            </div>
                ...
                <div id="interviews-section" className="space-y-4 animate-fade-in scroll-mt-20">
                    <div className="flex items-center justify-between">
                        <h3 className="text-lg font-black text-slate-800 flex items-center gap-2">
                            <Calendar className="h-5 w-5 text-primary" /> Upcoming Interviews
                        </h3>
                        {upcomingInterviews.length > 0 && (
                            <span className="px-2 py-1 bg-primary/10 text-primary text-[10px] font-bold rounded-full uppercase tracking-widest">
                    ...
                    {isInterviewsLoading ? (
                        <div className="flex items-center justify-center p-6 bg-white rounded-2xl border border-slate-100 border-dashed">
                            ...
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Checking...</span>
                        </div>
                    ) : upcomingInterviews.length > 0 ? (
                        <div className="grid gap-3 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
                            {upcomingInterviews.map((interview) => (
                                <div key={interview.id} className="group relative rounded-xl border border-slate-200 bg-white p-3 shadow-sm hover:shadow-md transition-all">
                                    <div className="flex items-center justify-between">
                                        <div className="truncate">
                                            <p className="text-[9px] font-black uppercase text-primary tracking-widest truncate">{interview.roundType}</p>
                                            <h4 className="text-xs font-black text-slate-800 truncate">{interview.companyName}</h4>
                                        </div>
                                        {interview.meetingLink ? (
                                            <a href={interview.meetingLink} target="_blank" rel="noreferrer" className="p-2 bg-primary text-white rounded-lg hover:bg-primary-dark">
                                                <Video className="h-3 w-3" />
                                            </a>
                                        ) : <div className="p-2 bg-slate-50 text-slate-400 rounded-lg"><MapPin className="h-3 w-3" /></div>}
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="flex items-center gap-3 p-4 bg-slate-50/50 rounded-2xl border border-slate-100 border-dashed">
                            ...
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">No active interview schedule</p>
                        </div>
                    )}
                </div>
