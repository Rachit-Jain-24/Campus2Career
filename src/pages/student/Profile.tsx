...
import { analyzeProfileWithAI, type ProfileAnalysis } from "../../lib/gemini";
...
export default function ProfilePage() {
        ...
        try {
            ...
            if (uploadError) {
                ...
                if (uploadError.message.includes("not found")) {
                    showToast("Storage not configured. Please contact admin.", 'error');
                } else {
            ...
            const updatedUser: any = { ...user, resumeUrl: downloadURL, resumeName: file.name };
            
            // TRIGGER AI ANALYSIS
            setUploadProgress(90);
            try {
                const analysis = await analyzeProfileWithAI(updatedUser);
                updatedUser.resumeAnalysis = analysis;
                updatedUser.readinessScore = analysis.careerStrength;
            } catch (aiErr) {
                console.error("AI Analysis failed after upload:", aiErr);
            }
    ...
    const handleSave = async (overrides = {}) => {
        ...
        try {
            ...
            showToast(`Save failed: ${err.message || 'Unknown error'}. Please try again.`, 'error');
        }
                        ...
                        <div className="space-y-4">
                            ...
                            <Card className="border-none shadow-xl bg-gradient-to-br from-slate-900 to-slate-800 text-white overflow-hidden relative">
                                ...
                                <CardHeader className="pb-2">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <Button 
                                            ...
                                            className="h-7 text-[10px] bg-white/10 hover:bg-white/20 text-white border-none"
                                        >
                                            {isUploading ? <RefreshCw className="h-3 w-3 animate-spin mr-1" /> : <RefreshCw className="h-3 w-3 mr-1" />}
                                            Re-Sync
                                        </Button>
                                    </div>
                                </CardHeader>
                                <CardContent className="space-y-6">
                                    {(() => {
                                        const analysis: ProfileAnalysis | undefined = user?.resumeAnalysis;
                                                ...
                                                <>
                                                    <div className="flex items-center gap-6">
                                                        <div className="relative shrink-0">
                                                            <svg className="h-24 w-24 transform -rotate-90">
                                                                ...
                                                                <circle
                                                                    ...
                                                                    strokeDashoffset={263.8 - (263.8 * analysis.careerStrength) / 100}
                                                                    className={cn(
                                                                        "transition-all duration-1000 ease-out",
                                                                        analysis.careerStrength > 75 ? "text-emerald-400" : analysis.careerStrength > 50 ? "text-yellow-400" : "text-orange-400"
                                                                    )}
                                                                />
                                                            </svg>
                                                            <div className="absolute inset-0 flex flex-col items-center justify-center">
                                                                <span className="text-2xl font-black">{analysis.careerStrength}</span>
                                                        ...
                                                        <div className="flex-1">
                                                            ...
                                                            <h3 className="text-lg font-bold leading-tight">{analysis.overallStatus}</h3>
                                                            <p className="text-xs text-white/60 mt-1">Market Readiness: <span className="text-emerald-400 font-bold">{analysis.marketReadiness}</span></p>
                                                        </div>
                                                    </div>

                                                    <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                                                        <p className="text-[10px] uppercase font-bold text-white/40 mb-1">AI Feedback</p>
                                                        <p className="text-xs text-white/80 leading-relaxed italic">"{analysis.feedback}"</p>
                                                    </div>

                                                    <div className="grid grid-cols-2 gap-3">
                                                        <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                                                            <p className="text-[10px] uppercase font-bold text-white/40 mb-1">Top Suggested Roles</p>
                                                            <p className="text-[10px] font-bold truncate text-emerald-400">{analysis.recommendedRoles?.slice(0, 2).join(', ')}</p>
                                                        </div>
                                                        <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                                                            <p className="text-[10px] uppercase font-bold text-white/40 mb-1">Missing Skills</p>
                                                            <p className="text-[10px] font-bold truncate text-yellow-400">{analysis.suggestedSkills?.slice(0, 2).join(', ')}</p>
                                                        </div>
                                                    </div>

                                                    <div className="text-[10px] text-white/40 text-center">
                                                        Analyzed by Campus2Career AI • Updated {new Date().toLocaleDateString()}
                                                    </div>
                                                </>
                                            );
                                        }

                                        // Fallback to calculation if no AI data yet
                                        let score = 0;
                                        ...
                                        return (
                                            <>
                                                ...
                                                <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                                                    <p className="text-[10px] uppercase font-bold text-white/40 mb-1">Local Analysis</p>
                                                ...
                                                <div className="text-[10px] text-white/40 text-center">
                                                    Click Re-Sync to activate Full AI Analysis
                                                </div>
                                            </>
                                        );
                                    })()}
